...
<main class="flex-1">
    <header class="sticky top-0 z-50 flex h-16 items-center justify-between border-b border-gray-100 bg-white/80 px-8 backdrop-blur-md">
        <h1 class="text-lg font-bold tracking-tight text-gray-900">
            Account Settings
        </h1>
    
        <a href="{{ route('welcome') }}" class="group flex items-center gap-2 rounded-full bg-green-700 px-5 py-2 shadow-sm transition-all hover:bg-green-800 hover:shadow-md active:scale-95 no-underline">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-white transition-transform group-hover:-translate-x-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2.5">
    ...
    <div class="p-8 w-full">
        ...
        <div class="bg-white border border-gray-200 shadow-sm rounded-xl overflow-hidden w-full">
            ...
            <div class="p-8 bg-gray-50/50 border-b border-gray-200 flex flex-col md:flex-row items-center gap-6 w-full">
                <div class="relative group">
                    ...
                    <label for="img_upload" class="absolute -bottom-2 -right-2 bg-white border border-gray-200 p-1.5 rounded-lg shadow-md cursor-pointer hover:text-[#057E2E] transition-all">
                ...
                <div class="text-center md:text-left">
                    ...
                    @if(auth()->user()->student_id)
                        <p class="text-xs text-gray-400 mt-1 uppercase tracking-widest">Student ID: {{ auth()->user()->student_id }}</p>
                    @elseif(auth()->user()->employee_id)
                        <p class="text-xs text-gray-400 mt-1 uppercase tracking-widest">Employee ID: {{ auth()->user()->employee_id }}</p>
                    @else
            ...
            <form method="POST" action="{{ route('profile.update') }}" enctype="multipart/form-data" class="divide-y divide-gray-100 w-full">
                @csrf
                @method('PATCH')

                <div class="p-8 w-full">
                    <h3 class="text-sm font-bold text-gray-400 uppercase tracking-wider mb-6">Personal Information</h3>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 w-full">
                        ...
                        <div class="space-y-1">
                            ...
                            <input 
                                ...
                                class="w-full border border-gray-300 rounded-lg px-4 py-2.5 focus:ring-2 focus:ring-[#057E2E] outline-none transition-all text-sm"
                                maxlength="11"
                                oninput="checkContactNumber(this)"
                            >
