<aside class="w-64 min-h-screen bg-[#057E2E] flex flex-col shadow-lg sticky top-0">
    ...
    <div class="p-6 border-b border-white/20 text-center space-y-3">
        ...
        <p class="text-xs text-white/70">{{ auth()->user()->role ?? 'N/A' }}</p>
...
<nav class="flex-1 overflow-y-auto p-4 text-sm space-y-1">
    <a href="{{ route('system-admin.index') }}" 
       ...
       {{ request()->routeIs('system-admin.index') ? 'bg-white text-[#057E2E] font-bold shadow-md' : 'text-white hover:bg-white/10' }}">
        <span>Dashboard</span>
    </a>
    <a href="{{ route('system-admin.users.index') }}" 
       class="flex items-center gap-2 px-4 py-2 rounded-lg font-medium transition
       {{ request()->routeIs('system-admin.users.index') ? 'bg-white text-[#057E2E] font-bold shadow-md' : 'text-white hover:bg-white/10' }}">
        <span>Users</span>
    </a>
</nav>
    <!-- Footer / School Year -->
    <div class="p-4 border-t border-white/20 text-center text-[10px] text-white/80 leading-relaxed uppercase tracking-wider">
        ...
        @endphp
        <p class="mt-1 opacity-60">School Year {{ $currentYear }}–{{ $nextYear }}</p>
    </div>
</aside>